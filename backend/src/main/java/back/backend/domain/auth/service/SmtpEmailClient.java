...
public class SmtpEmailClient {
    ...
    public SmtpEmailClient(EmailAuthProperties properties, JavaMailSender mailSender) {
    ...
    public void sendVerificationEmail(
        ...
        String purposeText = purpose == EmailVerificationPurpose.SIGNUP
                ? "회원가입" : "비밀번호 재설정";
        String html = """
                ...
                <div style="max-width:520px;margin:32px auto;background:#fff;border:1px solid #f8d9df;border-radius:24px;overflow:hidden">
                  ...
                  <div style="padding:32px"><h1 style="font-size:21px;margin:0 0 14px">%s 이메일 인증</h1>
                  <p style="line-height:1.7;color:#475569">아래 인증번호를 입력해 주세요.</p>
                  ...
                  <p style="font-size:12px;color:#94a3b8">인증번호는 %d분 동안 유효합니다.</p></div>
                </div></body></html>
                """.formatted(purposeText, HtmlUtils.htmlEscape(code), expirationMinutes);
        sendHtmlEmail(email, "[Plamingo] " + purposeText + " 인증번호", html);
    }
    ...
    public void sendTripInvitationEmail(
        ...
        sendHtmlEmail(email, "[Plamingo] " + tripTitle + " 여행에 초대받았어요", html);
    }
    ...
    private void sendHtmlEmail(String email, String subject, String html) {
        if (!StringUtils.hasText(properties.getFrom()) || mailSender == null) {
            throw new BusinessException(AuthErrorCode.EMAIL_SEND_FAILED);
        }
        try {
    ...
    private void sendAdminOtpWithSmtp(String email, String html) {
        sendHtmlEmail(email, "[Plamingo] 관리자 로그인 인증번호", html);
    }
